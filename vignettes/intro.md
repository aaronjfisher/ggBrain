...
The package also includes several aesthetic features, including:
...
* Separete fill scales for template images and for overlaying voxelwise statistic: The *ggplot2* package generally discourages the use of color to display different types of data, on different scales. This poses a challange for brain image figures where we want to use color to display tissue intensity of a template brain image, and also use color to show the value of a voxel-wise statistic (such as seed correlation) overlaid on top of the template. The template tissue intensities and the binary sign of the test statistics are on different scales, which goes against *ggplot*'s method of mapping only one scale to color. `ggBrain` handles this by hardcoding the coloring of the template image.
