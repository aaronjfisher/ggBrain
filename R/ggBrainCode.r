...
#'
#' \code{ggBrain} creates ggplot brain images with minimal user input. This function calls \code{getBrainFrame}, which generates a data frame for use in ggplot objects. The \code{getBrainFrame} function can also be directly accessed by the user, for a deeper control of aesthetics.
#' @param brains A list of 4-D arrays of brain images, with time on the fourth dimension. Alternatively, list of 3-D arrays, single 3-D, or a single 4-D array can be entered. 
...
#' @param brain_ind a vector of the same length as \code{mar}. The \eqn{j^{th}} element of \code{brain_ind} tells the index number which highlights the appropriate brain from the list \code{brains} to be plotted in the \eqn{j^{th}} panel.
#' @param time a vector of the same length as \code{mar}, used for plotting snapshots 4-D fMRI data over time. The \eqn{j^{th}} element of \code{time} tells the time index to be used for the \eqn{j^{th}} panel.
#' @param col_template a vector of colors to be used for displaying the template brain image.
#' @param type the type of brain image to be shown, either 'signed', 'positive', 'binary' or 'structural'. These types should be used when the objects in \code{brains} are, respectively, images of voxelwise statistics that can be positive or negative (i.e. seed correlations); images of voxelwise statistics that are all positive (i.e. p-values); binary masks to be shown on top of tempaltes; or structural brain images that do not contain voxelwise statistics. When setting \code{type='structural'} there is no need to input a template brain image.
#' @param binary_color the color to be used for plotting either binary masks or voxelwise, positive statistics (i.e. p-values). This argument will be used when \code{type} is set to either 'positive' or 'binary'.
...
#' @param center_coords whether the brains should be centered. If TRUE, axis titles will not be shown. If FALSE, axis titles will correspond to slice number
#' @param all_brain_and_time_inds_one a parameter specific to \code{getBrainFrame}, which can generally be ignored by the user. This arguments helps \code{getBrainFrame} be called from different contexts within \code{ggBrain}, particularly when contructing ggplot images for the template brain. If set to \code{TRUE}, all elements of \code{brain_ind} and \code{time} will be forced to equal 1.
...
#' dd+ scale_alpha(range=c(0,1))+labs(fill='Sign',alpha='Magnitude\n(opacity)')+
#'		theme_bw() +theme_no_ticks()
#' 
#' 
#' ###################
#' # use grid.arrange to show the seed mask and the 
#' # seed correlation.
#' # Since these are on different scales, they shouldn't
#' # be combined into one ggplot object
#' mar   =   c(1,2,3)
...
#' 
#' 
#' # We can also add a key to the above plot,
#' # using lines_mat
...
#' 	time=1:4)
#' dd + theme_black_bg()+ labs(fill='Sign',alpha='Magnitude\n(opacity)')+scale_alpha(range=c(0,1))
#' 
#'}
ggBrain<-function(brains,template=NULL,mar=1,mar_ind,row_ind=rep(1,length(mar_ind)),col_ind=rep(1,length(mar_ind)),col_template=rev(brewer.pal(8,'Greys')),type='signed',binary_color='darkred',fix_ratio=TRUE,tri_planar=FALSE,lines_color='white',center_coords=TRUE,lines_mat=NULL, ...){
